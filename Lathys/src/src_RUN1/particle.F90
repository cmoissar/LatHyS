   ...
   if(dst_planet < Spe%P%r_lim + alt_part_imp/Spe%ref%c_omegapi) then
     if ((diag_part_imp.ne.0).and.(abs(particule(nn)%char/particule(nn)%mass-1._dp/16._dp) < 0.01)) then
        ...
        if (dst_planet_av >= Spe%P%r_lim + alt_part_imp/Spe%ref%c_omegapi .and. particule(nn)%orig/= 4) then
            ...
            Vr0   = V0(1)*cos(phi0)*sin(theta0) + V0(2)*sin(theta0)*sin(phi0) + V0(3)*cos(theta0)  !!--- Radial velocity km/s
            ener  = 0.5d0  * particule(nn)%mass/particule(nn)%char * amu_pmass * dot_product(1.d3*V0,1.d3*V0) * JtoeV !!---Energy in eV
            ...
            flux_surf(i_phi,i_theta,1) = flux_surf(i_phi,i_theta,1) + N_ions/surf(i_phi,i_theta)   !!--- Has to be divided by the surface of 
                                                                                                             !!--- the cell.
