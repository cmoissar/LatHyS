...
contains
 ...
 subroutine reassemble_unform(run_name)
  ...
  write(filename,'(a3,i3.3,a1,a)')"c3_",0,'_',trim(run_name)
  ...
  write(filename,'(a3,i3.3,a1,a)')"p3_",0,'_',trim(run_name)
  ...
  write(msg,'(81a,(2a,i8),2(2a,i3),2(2a,3i3),(2a,i3),(2a,i2,a,i2,a))')&
       &ch10," ",(/("-",ii=1,79)/),&
  ...
  if (nb_procs > 1) then
   do iproc=2,nb_procs
    ...
    !--Create file name
    write(filename,'(a3,i3.3,a1,a)')"p3_",iproc-1,'_',trim(run_name)
    iunit = 1
    open(unit=iunit,file = filename, action = 'read', &
         status = 'old',form = 'unformatted')
