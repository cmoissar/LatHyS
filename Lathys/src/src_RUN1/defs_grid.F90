...
contains
 ...
 subroutine ncell_compute(ncx0,ncy0,ncz0)
  ...
  do ii=1,ct0z
   ...
   if (cl_z(w0).eq.0) then
	   ...
	   if (cl_z(w1).le.1) then
		print *,mpiinfo%me,' has a problem, too much process for grid size? clz ',cl_z
		stop
	   endif
   	   cl_z(w1)=cl_z(w1)-1
   	   cl_z(w0)=cl_z(w0)+1
  ...
  ncross = 2*8*int((ncm_tot(2)*ncm_tot(3) + ncm_tot(2)*ncm_tot(1) + ncm_tot(3)*ncm_tot(1))/nproc)
