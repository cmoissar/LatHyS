 ...
 do kk = 1,ncm(3)-1
   do jj = 1,ncm(2)-1
     do ii = 1,ncm(1)-1
          ...
          if ((radius <= Spe%P%radius+200./Spe%ref%c_omegapi).and.(radius >= maxval(Altitude(:,1,1)))) then
...
!          if (radius <= Distance(nr)+1000.*Spe%P%radius/(Spe%ref%c_omegapi)) then
 ...
 !!! check up to there
           ...
           ! Find the good phi
           do while ((find_phi == .false.).and.(n_phi <= np))
           ...
           enddo
           ! Find the good theta
           do while ((find_theta == .false.).and.(n_theta <= nt))
	     ...
	     do while ((find_alt == .false.).and.(n_alt <= nr))
