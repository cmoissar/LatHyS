...
!--Reading variables
  ...
  write(filename,'(a3,i3.3,a1,2a)')"p3_",0,'_',trim(run_name),".nc"
!--Inquire if the file exists
 ...
 do ii=0,nb_procs-1
 !--Create file name 
   write(filename,'(a3,i3.3,a1,2a)')"p3_",ii,'_',trim(run_name),".nc"
 !--Inquire if the file exists
   inquire( file=trim(filename), exist=file_e )
   !--No file: return
   if(.not.(file_e)) then
...
!!-diagnostic of recording O+ ions in the plume
  ...
  write(write_name,'(a19,i3.3,a5)')"Particle_selection_",rank,"_.dat"
iunit=10
