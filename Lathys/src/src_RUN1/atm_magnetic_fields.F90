...
contains
 ...
 integer ::i,j,k,n
  ...
  do n=2,l 
	do k=0,l-n 
		p(k+n,k)=(x*(2._dp*real(k+n,dp)-1._dp)*p(k+n-1,k)/sqrt(2._dp*real(k,dp)/real(n,dp)+1._dp)-&
			(real(2*k+n,dp)-1._dp)*p(k+n-2,k)/sqrt(2._dp*real(k,dp)/real(n,dp)+1._dp)/sqrt(2._dp*real(k,dp)/real(n-1,dp)+1._dp))/real(n,dp)
		d_p(k+n,k)=(xpdx*(2._dp*real(k+n,dp)-1._dp)*d_p(k+n-1,k)/sqrt(2._dp*real(k,dp)/real(n,dp)+1._dp)-&
			(real(2*k+n,dp)-1._dp)*d_p(k+n-2,k)/sqrt(2._dp*real(k,dp)/real(n,dp)+1._dp)/sqrt(2._dp*real(k,dp)/real(n-1,dp)+1._dp))/real(n,dp)
	enddo
  enddo
  endif
  d_p=(d_p-p)*1E3
...
subroutine add_multipole_generic(Bfield,ncm,Spe,gstep,s_min_loc,n_sph,g,h)
  ...
  real(dp) :: radius,ss(3),b_sph(3),phi,theta,rp,b_xyz(3),satur
  ...
  do kk = 1,ncm(3)-1
   ...
   do jj = 1,ncm(2)-1
    ...
    do ii = 1,ncm(1)-1
     ...
     b_sph(:)=0.
     radius = sqrt(dot_product(ss,ss))
     lmax=min(int(100.*(Spe%P%radius/radius)**3),int(100.*(radius/Spe%P%radius)**20)) ! limits the precision as a function of distance
     lmax=max(lmax,1);  lmax=min(lmax,n_sph)
     if ((radius >= .85*Spe%P%radius).and.(lmax.gt.0)) then
	    phi=atan2(ss(2),ss(1));  theta=atan2(sqrt(ss(1)**2+ss(2)**2),ss(3))
	...
	b_sph(3)=0.
        ...
        b_xyz=convert(b_sph,theta,phi)
	Bfield%x(ii,jj,kk)=Bfield%x(ii,jj,kk)+b_xyz(1);Bfield%y(ii,jj,kk)=Bfield%y(ii,jj,kk)+b_xyz(2);Bfield%z(ii,jj,kk)=Bfield%z(ii,jj,kk)+b_xyz(3)
     endif
    enddo
   enddo
  enddo
