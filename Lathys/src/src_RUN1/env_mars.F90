...
contains
 ...
 use defs_parametre
  ...
  species%S(H)%vxs = 400._dp/species%ref%alfvenspeed !--  H+  ! 12.18
  ...
  species%betae = 1.08_dp        ! 1.08 !               0.084
...
'<Profile>Krasnopolsky et al, 2002,Kim et al, 1998</Profile>'//char(10)//'<ModelURL>http://dx.doi.org/10.1029/2001JE001809</ModelURL>'
  density_CO2  => density(:,:,:,CO2)
		...
		write(atmosphere%species(CO2)%description,'(a)') '<Profile>Ma et al, 2004</Profile>'
  density_H    => density(:,:,:,Hn)
		...
		write(atmosphere%species(Hn)%description,'(a)') '<Profile>Krasnopolsky et al, 1993</Profile>'
...
!  atmosphere%exc_reactions(4)%cross_section = zero
  ...
  atmosphere%ei_reactions(1)%coeff(1:5) = (/-1143.33,323.767,-35.0431,1.69073,-0.0306575/)
  write(atmosphere%ei_reactions(1)%description,'(A)') '<ProcessModel>ISSIChallenge</ProcessModel>'
  ...
  write(atmosphere%ei_reactions(2)%description,'(A)') '<ProcessModel>ISSIChallenge</ProcessModel>'
 ...
 subroutine exosphere_mars(Spe,ncm,gstep,s_min_loc,resistivity)
  ...
  use defs_parametre,only :dt,exospherename,atmospherename
   ...
   real(dp),parameter :: d0_O = 2.33e13,e0_O = one/12.27,&
        ...
        &                d2_O = 1.56e4 ,e2_O = one/696.9,&
        &                d3_O = 2.926e3 ,e3_O = one/2891.,&
        ...
        &                d0_CO2 = 5.88e18,e0_CO2 = one/7.00, &
        &                d1_CO2 = 3.55e13, e1_CO2 = one/16.67
 ...
 subroutine photoproduction_mars(Spe,ncm,gstep,s_min_loc,atmosphere)
  ...
  atmosphere%F107  = 130._dp  ! daily F10.7 (e.g. 74)
  atmosphere%F107_Avg = 130._dp  ! 81 day average F10.7 (F10.7A) (e.g. 86)
...
!======V environment dependent code from here V===============
  ...
  do k=min_i(3),max_i(3)+1
	...
	(prod_O(i,j,k)+k5*density_CO2p(i,j,k)*density_O(i,j,k))/(k6*density_CO2(i,j,k)+k8*density_H(i,j,k))
	if (radius.gt.(300./Spe%ref%c_omegapi+Spe%P%r_lim)**2) density_Op(i,j,k)=0.

 	density_O2p(i,j,k) =(k4*density_CO2p(i,j,k)*density_O(i,j,k)+k6*density_CO2(i,j,k)*density_Op(i,j,k))/k7 
        density_O2p(i,j,k) =0.5*(sqrt((density_CO2p(i,j,k)+density_Op(i,j,k))**2+4.0_dp*density_O2p(i,j,k))-(density_CO2p(i,j,k)+density_Op(i,j,k)))	
...
!	    Alt_GEO_min = (radius-Spe%P%radius)*Spe%ref%c_omegapi
 ...
 !  	      density_O2p(ii,jj,kk) = nO2p(iHoru,ialt,nTime)   	    
   	   ...
   	   ! print *,'ialt_min, Alt_min,ialt_max,Alt_max',ialt_min,Altitude(iHoru,ialt_min,nTime),ialt_max,Altitude(iHoru,ialt_max,nTime)
   	    
   	    
   !	    if (ialt_min > ialt_max-1) then
   !	     do ialt = ialt_min,ialt_max -1
   !	   ! if (ss(1) > Spe%P%centr(1)+7.*Spe%P%radius/8.) then
   !	   !   print *,'x_cdr,y_cdr,z_cdr,Lat_GCM,ilat,nOp :',x_cdr,y_cdr,z_cdr,ilon,Lat_GEO,ilat,nOp(iHoru,ialt,nTime) 
   !	   ! endif
   !	   density_Op(ii,jj,kk) = density_Op(ii,jj,kk) + nOp(iHoru,ialt,nTime)*(Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   density_O2p(ii,jj,kk) = density_O2p(ii,jj,kk) + nO2p(iHoru,ialt,nTime)*(Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   density_CO2p(ii,jj,kk) = density_CO2p(ii,jj,kk) + nCO2p(iHoru,ialt,nTime)*(Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   density_Hp(ii,jj,kk) = density_Hp(ii,jj,kk) + nHp(iHoru,ialt,nTime)*(Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   ! density_Op(ii,jj,kk) = density_Op(ii,jj,kk) + (nOp(iHoru,ialt,nTime)+nOp(iHoru,ialt+1,nTime))/2.*&
   !	   ! 		(Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   ! density_Hp(ii,jj,kk) = density_Hp(ii,jj,kk) + (nHp(iHoru,ialt,nTime)+nHp(iHoru,ialt+1,nTime))/2.*&
   !	   !             (Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   ! density_CO2p(ii,jj,kk) = density_CO2p(ii,jj,kk) + (nCO2p(iHoru,ialt,nTime)+nCO2p(iHoru,ialt+1,nTime))/2.*&
   !	   !             (Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	   ! density_O2p(ii,jj,kk) = density_O2p(ii,jj,kk) + (nO2p(iHoru,ialt,nTime)+nO2p(iHoru,ialt+1,nTime))/2.*&
   !	   !             (Altitude(iHoru,ialt+1,nTime)-Altitude(iHoru,ialt,nTime))
   !	    enddo
   !	    
   !	    density_Op(ii,jj,kk) = density_Op(ii,jj,kk)/(Altitude(iHoru,ialt_max,nTime)-Altitude(iHoru,ialt_min,nTime))
   !	    density_Hp(ii,jj,kk) = density_Hp(ii,jj,kk)/(Altitude(iHoru,ialt_max,nTime)-Altitude(iHoru,ialt_min,nTime))
   !	    density_O2p(ii,jj,kk) = density_O2p(ii,jj,kk)/(Altitude(iHoru,ialt_max,nTime)-Altitude(iHoru,ialt_min,nTime))
   !	    density_CO2p(ii,jj,kk) = density_CO2p(ii,jj,kk)/(Altitude(iHoru,ialt_max,nTime)-Altitude(iHoru,ialt_min,nTime))
   !	  !   density_Op(ii,jj,kk) = density_Op(ii,jj,kk)/(gstep(1)*Spe%ref%c_omegapi*(ialt_max-ialt_min))
   !	  !   density_Hp(ii,jj,kk) = density_Hp(ii,jj,kk)/(gstep(1)*Spe%ref%c_omegapi*(ialt_max-ialt_min))
   !	  !   density_O2p(ii,jj,kk) = density_O2p(ii,jj,kk)/(gstep(1)*Spe%ref%c_omegapi*(ialt_max-ialt_min))
   !	  !   density_CO2p(ii,jj,kk) = density_CO2p(ii,jj,kk)/(gstep(1)*Spe%ref%c_omegapi*(ialt_max-ialt_min))
   !	    else
   !	      density_Op(ii,jj,kk) = nOp(iHoru,ialt_min,nTime)
   !	      density_Hp(ii,jj,kk) = nHp(iHoru,ialt_min,nTime)
   !	      density_CO2p(ii,jj,kk) = nCO2p(iHoru,ialt_min,nTime)
   !	      density_O2p(ii,jj,kk) = nO2p(iHoru,ialt_min,nTime)
   !	    endif  
   !	 
   !	  !density_Op(ii,jj,kk) = sum(nOp(iHoru,ialt_min:ialt_max,nTime)*altitude(iHoru,ialt_min:ialt_max,nTime))/sum(Altitude(iHoru,ialt_min:ialt_max,nTime))
   !	  !density_O2p(ii,jj,kk) = sum(nO2p(iHoru,ialt_min:ialt_max,nTime)*altitude(iHoru,ialt_min:ialt_max,nTime))/sum(Altitude(iHoru,ialt_min:ialt_max,nTime))
   !	  !density_Hp(ii,jj,kk) = sum(nHp(iHoru,ialt_min:ialt_max,nTime)*altitude(iHoru,ialt_min:ialt_max,nTime))/sum(Altitude(iHoru,ialt_min:ialt_max,nTime))
   !	  !density_CO2p(ii,jj,kk) = sum(nCO2p(iHoru,ialt_min:ialt_max,nTime)*altitude(iHoru,ialt_min:ialt_max,nTime))/sum(Altitude(iHoru,ialt_min:ialt_max,nTime))
   !	 
   !	  print *, radius, Altitude(iHoru,ialt_min,nTime),Altitude(iHoru,ialt_max,nTime),density_O2p(ii,jj,kk),nO2p(iHoru,ialt_min+int((ialt_max-ialt_min)/2),nTime)
   !	!    print *, 'nOp i,j,k',nOp(iHoru,ialt,nTime),ii,jj,kk
!!               density_O2i(ii,jj,kk) = nO2p(iHoru,ialt_max,nTime)/3.
!!               density_Hi(ii,jj,kk) = nHp(iHoru,ialt_max,nTime)
!!               density_CO2i(ii,jj,kk) = nCO2p(iHoru,ialt_max,nTime)/3.
!!               density_Oi(ii,jj,kk) = nOp(iHoru,ialt_max,nTime)
...
k4=1.64E-10;k5=9.6E-11;k6=1.1E-9;k7=7.38E-8;k8=2.5E-11*sqrt(300.+2000./16.)
 !coefficients des reactions (pour k8 cf these de O wittasse)
	...
	if (no_coll) then
		if (atmosphere%species(l)%name.eq.("CO2+      ")) t3=prod_CO2(i,j,k)-(k4+k5)*density_O(i,j,k)*density_CO2p(i,j,k)
		if (atmosphere%species(l)%name.eq.("O+        ")) then
			t3=prod_O(i,j,k)+k5*density_CO2p(i,j,k)*density_O(i,j,k)-(k6*density_CO2(i,j,k)+k8*density_H(i,j,k))*density_Op(i,j,k)
			if (r2.gt.(300./Spe%ref%c_omegapi+Spe%P%radius)**2) t3=0.
		endif
		if (atmosphere%species(l)%name.eq.("O2+       ")) then
			t3 =(k4*density_CO2p(i,j,k)*density_O(i,j,k)+k6*density_CO2(i,j,k)*density_Op(i,j,k))&
				-k7*density_O2p(i,j,k)*(density_O2p(i,j,k)+density_Op(i,j,k)+density_CO2p(i,j,k))
			if ((rb.lt.(Spe%P%radius)**2).and.((float(i-1)*gstep(1)-s_cen(1)).gt.0)) t3=0.
		endif
	else 
		t3=0.
	endif	
...
subroutine Iono_loaded_mars(i,j,k,atmosphere,l,t3,r2,rb,Spe,s_cen)
  ...
  prod_unit=1E6/Spe%ref%density*dt*Spe%ref%inv_gyro
    t3 = 0.
! NB : we dviide by prod_unit because in the atm_ionosphere production  we multiply t3 by prod_unit to have the production in cm-3 during one time step
!		if (atmosphere%species(l)%name.eq.("CO2+      ").and.(density_CO2p(i,j,k)<density_CO2i(i,j,k))) then
! 
!		t3=(density_CO2i(i,j,k) - density_CO2p(i,j,k))/prod_unit
!                !print *,'CO2+ prod',density_CO2p(i,j,k),atmosphere%species(1)%density_ini(i,j,k),t3
!                endif
!		if (atmosphere%species(l)%name.eq.("O+        ").and.(density_Op(i,j,k)<density_Oi(i,j,k))) then
!		        t3 = (density_Oi(i,j,k)-density_Op(i,j,k))/prod_unit
!                !print *,'O+ prod',density_Op(i,j,k),atmosphere%species(2)%density_ini(i,j,k),t3
!                endif
!		if (atmosphere%species(l)%name.eq.("O2+       ").and.(density_O2p(i,j,k)<density_CO2i(i,j,k))) then
!			 t3 = (density_CO2i(i,j,k)-density_O2p(i,j,k))/prod_unit
!                !print *,'O2+ prod',density_O2p(i,j,k),atmosphere%species(4)%density_ini(i,j,k),t3
!                endif
!
!		if (atmosphere%species(l)%name.eq.("H+        ").and.(density_Hp(i,j,k)<density_Hi(i,j,k))) then
!		        t3 = (density_Hi(i,j,k)-density_Hp(i,j,k))/prod_unit	
!                !print *,'H+ prod',density_Hp(i,j,k),atmosphere%species(3)%density_ini(i,j,k),t3
!                endif
!		print *,'Prod iono',t3*prod_unit
       ...
       do kk = 1,ncm(3)-1
        do jj = 1,ncm(2)-1
         ...
         ! do iii=-4,4
          ...
          ss(1) = (real((ii-1),dp))*gstep(1) + s_min_loc(1)      
          radius = sqrt(dot_product(ss-Spe%P%centr,ss-Spe%P%centr))
          if (radius <= altitude(npt_alt)) then
          x_cdr = -(ss(1)-Spe%P%centr(1))
          y_cdr = -(ss(2)-Spe%P%centr(2))
          z_cdr =  (ss(3)-Spe%P%centr(3))
...
! check conditions to obtain the good angles
          ...
          do while ((find_alt == .false.).and.(n_alt <= npt_alt))
          ...
          enddo
          ! Find the good phi
          do while ((find_phi == .false.).and.(n_phi <= npt_lon))
          ...
          enddo
          ! Find the good theta
          do while ((find_theta == .false.).and.(n_theta <= npt_lat))
         ...
         if (find_phi == .false.) n_phi = 1
         if (find_theta == .false.) n_theta = 1
         
         if (find_alt ==.true.)  density_O(ii,jj,kk) = densO(n_alt,n_theta,n_phi)
         if (find_alt ==.true.)  density_CO2(ii,jj,kk) = densCO2(n_alt,n_theta,n_phi)         
