...
contains
 ...
 subroutine create_file_name_io(name_file,filwrt,me)
  ...
  write(name_file,'(a5,i4.4,a1,a)')"Atm3_",me,'_',trim(filwrt)
#ifdef HAVE_NETCDF
...
!#ifdef HAVE_NETCDF 
 ...
 use environment,only : atmosphere
  ...
  call set_global_attribute_cdf(ncid,"Ionosphere")
 ...
 ! if (atmosphere%species(i)%iono == .FALSE.) then  
  ...
  call test_cdf(stId); ii = ii+1
  stId = nf90_put_var(ncid, varid(ii), atmosphere%species(i)%density)
  call test_cdf(stId); ii = ii+1
 ! endif
  enddo       
 ...
 !print *,'test2'
  ...
  __WRT_DEBUG_OUT("wrt_iono")
 end subroutine wrt_iono

!#endif

end module diag_iono
